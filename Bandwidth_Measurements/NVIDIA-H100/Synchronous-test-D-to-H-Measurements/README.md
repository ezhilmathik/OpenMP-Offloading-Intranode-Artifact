# Synchronous Device-to-Host Bandwidth — NVIDIA H100

Benchmarks accompanying the paper *[PAPER TITLE]*.

This directory measures **synchronous device-to-host (D-to-H) transfer
...
The companion directory `../Synchronous-test-H-to-D-Measurements/` measures
the opposite direction with an identical methodology. Other vendors:
...
|---|---|---|
| `v1` | Transfer from a single GPU to host | 64 GB/s (PCIe Gen5 x16) |
| `v2` | Concurrent transfer from two GPUs to host | 128 GB/s |
...
| `plot_h100_sync_dtoh.py` | Aggregates all runs and produces the paper figure |
...
|---|---|---|---|
| *[Figure N]*: OpenMP offload vs CUDA, sync D-to-H on H100 | `python plot_h100_sync_dtoh.py` | seconds, no GPU needed | `plots_final/[filename].pdf` |
...
| *[Any claim comparing D-to-H against H-to-D]* | also run the H-to-D directory | ~5 node-h total | both figures |
