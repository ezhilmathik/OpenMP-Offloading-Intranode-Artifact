#!/usr/bin/env python3
"""
plot_sync_dtoh.py — Bandwidth vs transferred size for versions v1, v2
                    (Sync D-to-H transfer, H100)

Usage:
    python3 plot_sync_dtoh.py
...
GAP_TEXTBOX_LOC  = (0.02, 0.25)   # axis fraction; (x, y), bottom-left origin
...
#    ax1.set_title("NVIDIA H100", fontsize=16, fontweight="bold", pad=12)
    ax1.set_title("NVIDIA H100 - DtoH: synchronous", fontsize=16, fontweight="bold", pad=12)    
    ...
    for ver in VERSIONS_TO_PLOT:
        ...
        if st is None:
            continue
        gpu_label = SERIES[("omp_off", ver)][0].split("(")[1].rstrip(")")  # "1 GPU"
    ...
    #    v1 ->  64 GB/s   (one PCIe Gen5 x16 D-to-H link)   blue
    #    v2 -> 128 GB/s   (two D-to-H links in parallel)    green
