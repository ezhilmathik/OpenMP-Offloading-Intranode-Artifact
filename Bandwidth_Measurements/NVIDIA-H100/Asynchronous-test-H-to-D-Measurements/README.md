# Asynchronous Host-to-Device Bandwidth — NVIDIA H100
...
against **native CUDA**, for transfers targeting one GPU and two GPUs
concurrently. Transfers are issued without blocking, so that data movement
can overlap rather than serialise.

Companion directories: `../Synchronous-test-H-to-D-Measurements/` and
`../Synchronous-test-D-to-H-Measurements/` measure the blocking equivalents.
Other vendors: `../../AMD-MI250X/` and `../../Intel-1550/`.
...
|---|---|---|
| `v1` | Asynchronous transfer to a single GPU | 64 GB/s (PCIe Gen5 x16) |
| `v2` | Concurrent asynchronous transfer to two GPUs | 128 GB/s |
...
| `plot_h100_async_htod.py` | Aggregates all runs and produces the paper figure |
...
```bash
python plot_h100_async_htod.py
```
...
|---|---|---|---|
| *[Figure N]*: OpenMP offload vs CUDA, async H-to-D on H100 | `python plot_h100_async_htod.py` | seconds, no GPU needed | `plots_final/[filename].pdf` |
...
| *[Any claim comparing async against sync]* | also run `../Synchronous-test-H-to-D-Measurements/` | ~5 node-h total | both figures |
...
not actually overlapping. Check that the host allocation is page-locked, since
transfers from pageable memory fall back to blocking behaviour.
