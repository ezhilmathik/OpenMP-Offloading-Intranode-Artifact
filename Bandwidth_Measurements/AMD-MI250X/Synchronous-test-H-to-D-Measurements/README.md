# Synchronous Host-to-Device Bandwidth — AMD MI250X

Benchmarks accompanying the paper *[PAPER TITLE]*.

This directory measures **synchronous host-to-device (H-to-D) transfer
bandwidth** on an AMD Instinct MI250X node, comparing **OpenMP target
offload** against **native HIP**.

Companion directories in `../`: `Synchronous-test-D-to-H-Measurements/`,
...
|---|---|---|
| `v1` | Transfer to a single GCD | 36 GB/s |
| `v2` | Concurrent transfer to two GCDs on separate cards | 72 GB/s |

Each is implemented twice — once in OpenMP target offload, once in HIP —
...
`plot_mi250x_sync_htod.py` and drawn as dashed reference lines.
...
| `plot_mi250x_sync_htod.py` | Aggregates all runs and produces the paper figure |
...
|---|---|---|---|
| *[Figure N]*: OpenMP offload vs HIP, sync H-to-D on MI250X | `python3 plot_mi250x_sync_htod.py` | seconds, no GPU needed | `plots_final/mi250x_sync_htod.pdf` |
