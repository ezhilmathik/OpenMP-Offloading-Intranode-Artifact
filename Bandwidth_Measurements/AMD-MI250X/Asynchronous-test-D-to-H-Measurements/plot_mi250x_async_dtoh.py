#!/usr/bin/env python3
"""
plot_dtoh.py — Bandwidth vs transferred size for versions v1, v2
               (Async D-to-H transfer, LUMI / MI250X)
...
The x-axis reports the transferred data size, derived from N:
    bytes = N * DTYPE_BYTES
...
# bytes = N * DTYPE_BYTES.  Set UNIT_BASE to 1000 for decimal GB (matches a
# bandwidth computed as bytes/1e9/sec) or 1024 for GiB — it must match whatever
# convention the benchmark used for bandwidth_gbs, or the axes disagree.
#
...
OUTPUT_NAME      = "mi250x_async_dtoh.pdf"
VERSIONS_TO_PLOT = ["v1", "v2"]

# Theoretical host-link bandwidth, one direction (GB/s).
#   v1 = 1 GPU  -> a single D-to-H link
#   v2 = 2 GPUs -> two independent D-to-H links driven in parallel
...
# ── HIP-vs-OpenMP gap box ────────────────────────────────────────────────────
# Prints the mean absolute bandwidth difference between HIP and OpenMP Off.
...
GAP_BASELINE_FW  = "hip"          # framework compared against omp_off ("hip"/"cuda")
GAP_BASELINE_LBL = "HIP"          # how that framework is named in the box
...
def main():
    ...
    ax1.set_title("AMD MI250X (1 GCD) - DtoH: asynchronous", fontsize=16, fontweight="bold", pad=12)
    ...
    # ── HIP vs OpenMP gap: quantify only, no shading ───────────────────────
    ...
    #    v1 -> 36 GB/s   (one D-to-H link)    blue
    #    v2 -> 72 GB/s   (two D-to-H links in parallel)   green
