# Synchronous Device-to-Host Bandwidth — Intel Data Center GPU Max 1550
...
|---|---|---|
| `v1` | Transfer from a single device to host | 64 GB/s |
| `v2` | Concurrent transfer from two devices to host | 128 GB/s |
...
| `plot_pvc_sync_dtoh.py` | Aggregates all runs and produces the paper figure |
...
```bash
python3 plot_pvc_sync_dtoh.py
```

Globs all `results_*/timings.csv`, averages each (framework, version, size)
triple across the five repetitions, writes `plots_final/pvc_sync_dtoh.pdf`, and
...
|---|---|---|---|
| *[Figure N]*: OpenMP offload vs SYCL, sync D-to-H on PVC | `python3 plot_pvc_sync_dtoh.py` | seconds, no GPU needed | `plots_final/pvc_sync_dtoh.pdf` |
