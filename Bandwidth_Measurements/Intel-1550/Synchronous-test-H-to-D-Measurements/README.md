# Synchronous Host-to-Device Bandwidth — Intel Data Center GPU Max 1550
...
|---|---|---|
| `v1` | Transfer to a single device | 64 GB/s |
| `v2` | Concurrent transfer to two devices | 128 GB/s |

Each is implemented twice — once in OpenMP target offload, once in SYCL —
...
| `plot_pvc_sync_htod.py` | Aggregates all runs and produces the paper figure |
...
```bash
python3 plot_pvc_sync_htod.py
```

Globs all `results_*/timings.csv`, averages each (framework, version, size)
triple across the five repetitions, writes `plots_final/pvc_sync_htod.pdf`, and
...
|---|---|---|---|
| *[Figure N]*: OpenMP offload vs SYCL, sync H-to-D on PVC | `python3 plot_pvc_sync_htod.py` | seconds, no GPU needed | `plots_final/pvc_sync_htod.pdf` |
