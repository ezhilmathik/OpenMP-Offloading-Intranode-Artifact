# Asynchronous Host-to-Device Bandwidth — Intel Data Center GPU Max 1550
...
|---|---|---|
| `v1` | Asynchronous transfer to a single device | 64 GB/s |
| `v2` | Concurrent asynchronous transfer to two devices | 128 GB/s |
...
| `plot_pvc_async_htod.py` | Aggregates all runs and produces the paper figure |
...
```bash
python3 plot_pvc_async_htod.py
```

Globs all `results_*/timings.csv`, averages each (framework, version, size)
triple across the five repetitions, writes `plots_final/pvc_async_htod.pdf`, and
...
|---|---|---|---|
| *[Figure N]*: OpenMP offload vs SYCL, async H-to-D on PVC | `python3 plot_pvc_async_htod.py` | seconds, no GPU needed | `plots_final/pvc_async_htod.pdf` |
