    ...
    if (result != 0) {
        ...
        */
        cerr << "CUDA error in " << func
             << ": "
             /*
             DPCT1009:53: SYCL uses exceptions to report errors and does not use
...
int main(int argc, char *argv[]) {
    ...
    /*
    DPCT1093:54: The "0" device may be not the one intended for use. Adjust the
    ...
    */
    ...
    /*
    DPCT1093:55: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    ...
    if (!d_B) {
        /*
        DPCT1093:56: The "0" device may be not the one intended for use. Adjust
    ...
    /*
    DPCT1093:57: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    checkCuda(DPCT_CHECK_ERROR(dpct::select_device(0)), "cudaSetDevice 0");
    checkCuda(
    ...
    /*
    DPCT1093:58: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    checkCuda(DPCT_CHECK_ERROR(dpct::select_device(1)), "cudaSetDevice 1");
    checkCuda(
    ...
    for (int w=0; w<2; w++)
      {
        /*
        DPCT1093:59: The "0" device may be not the one intended for use. Adjust
        the selected device if needed.
        */
        checkCuda(DPCT_CHECK_ERROR(dpct::select_device(0)), "cudaSetDevice 0");
        /*
        DPCT1124:60: cudaMemcpyAsync is migrated to asynchronous memcpy API.
        ...
        */
        ...
        /*
        DPCT1093:61: The "1" device may be not the one intended for use. Adjust
        the selected device if needed.
        */
        checkCuda(DPCT_CHECK_ERROR(dpct::select_device(1)), "cudaSetDevice 1");
        /*
        DPCT1124:62: cudaMemcpyAsync is migrated to asynchronous memcpy API.
        While the origin API might be synchronous, it depends on the type of
        operand memory, so you may need to call wait() on event return by memcpy
        API to ensure synchronization behavior.
        */
    ...
    #pragma omp parallel sections
    {
        ...
        {
            /*
            DPCT1093:63: The "0" device may be not the one intended for use.
            Adjust the selected device if needed.
            */
            checkCuda(DPCT_CHECK_ERROR(dpct::select_device(0)),
                      "cudaSetDevice 0");
            /*
            DPCT1124:64: cudaMemcpyAsync is migrated to asynchronous memcpy API.
            While the origin API might be synchronous, it depends on the type of
        ...
        {
            /*
            DPCT1093:65: The "1" device may be not the one intended for use.
            Adjust the selected device if needed.
            */
            checkCuda(DPCT_CHECK_ERROR(dpct::select_device(1)),
                      "cudaSetDevice 1");
            /*
            DPCT1124:66: cudaMemcpyAsync is migrated to asynchronous memcpy API.
            While the origin API might be synchronous, it depends on the type of
            operand memory, so you may need to call wait() on event return by
            memcpy API to ensure synchronization behavior.
            */
    ...
    if (!h_verifyA || !h_verifyB) {
        ...
        /*
        DPCT1093:67: The "0" device may be not the one intended for use. Adjust
        the selected device if needed.
        */
        checkCuda(DPCT_CHECK_ERROR(dpct::select_device(0)), "cudaSetDevice 0");
            checkCuda(DPCT_CHECK_ERROR(
                      ...
                      "cudaFree d_A");
        /*
        DPCT1093:68: The "1" device may be not the one intended for use. Adjust
        the selected device if needed.
        */
        checkCuda(DPCT_CHECK_ERROR(dpct::select_device(1)), "cudaSetDevice 1");
            checkCuda(DPCT_CHECK_ERROR(
                      ...
                      "cudaFree d_B");
        dpct::get_current_device().destroy_queue(stream[0]);
            dpct::get_current_device().destroy_queue(stream[1]);
        free(h_A); free(h_B);
        return EXIT_FAILURE;
    }
    ...
    /*
    DPCT1093:69: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    checkCuda(DPCT_CHECK_ERROR(dpct::select_device(0)), "cudaSetDevice 0");
    checkCuda(DPCT_CHECK_ERROR(dpct::get_in_order_queue()
              ...
              "verify D→H d_A");
    /*
    DPCT1093:70: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    checkCuda(DPCT_CHECK_ERROR(dpct::select_device(1)), "cudaSetDevice 1");
    checkCuda(DPCT_CHECK_ERROR(dpct::get_in_order_queue()
    ...
    free(h_verifyA); free(h_verifyB);
    /*
    DPCT1093:71: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    checkCuda(DPCT_CHECK_ERROR(dpct::select_device(0)), "cudaSetDevice 0");
        checkCuda(
            ...
            "cudaFree d_A");
    /*
    DPCT1093:72: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    checkCuda(DPCT_CHECK_ERROR(dpct::select_device(1)), "cudaSetDevice 1");
        checkCuda(
