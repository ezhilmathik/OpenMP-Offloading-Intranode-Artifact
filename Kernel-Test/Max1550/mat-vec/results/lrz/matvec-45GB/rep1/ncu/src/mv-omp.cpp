...
//
// Counterpart of mv-cuda.cu: same N, same data (A = 1, 2, 3, ...; x = 1, 2, 3, 1, ...),
// same timing, every entry of y checked exactly against the CPU. Kernels:
//   matrix_vector_multiply       your kernel: one thread per row
//   matrix_vector_multiply_team  one team per row, the column loop split across the
//                                team's threads with a reduction: OpenMP's way of
//                                writing CUDA's coalesced warp-per-row
