...
#   KERNELS     matmul only: which native kernels to run, comma separated
#               (naive, naive1d, tiled32, regtile64, regtile128); default all.
#               KERNELS=tiled32 keeps only the kernel the comparison uses, which
#               makes the long Intel runs fit the walltime.
...
#   KEEP_VTUNE  1 = keep the raw VTune result 0   (several GB per run; the
#                   directories                   exported CSV is always kept)
...
#   MEM_OPT     Slurm memory request            --mem=0 (the whole node);
#                                             MEM_OPT=none omits the flag
...
# The host arrays are as large as the device footprint (~55 GB at the top size),
# so a job needs the node's memory, not Slurm's default share per core.
MEM_OPT="${MEM_OPT:---mem=0}"
...
write_meta() {
    local d=$1 gb=$2 N=$3 stage=$4
    {
        ...
        echo "rep: $REP"
        echo "reps: $REPS"
        [[ -n ${KERNELS:-} ]] && echo "kernels: $KERNELS"
        echo "date: $(date -Is)"
        echo "host: $(hostname)"
    ...
    for tag in "$NAT" omp; do
        ...
        { echo "=== vtune $tag"
          if [[ $tag == omp ]]; then OMP_TARGET_OFFLOAD=MANDATORY "$VTUNE" -collect gpu-hotspots \
                 ...
                 -knob profiling-mode=characterization -r "vt_$tag" -- "$bin" "$N" 1 ${KERNELS:+"$KERNELS"}
          fi; } >> vtune.out 2>&1
        "$VTUNE" -report hotspots -group-by computing-task -r "vt_$tag" \
                 -format=csv -csv-delimiter=comma > "vtune_$tag.csv" 2>> vtune.out
        if [[ -s vtune_$tag.csv ]]; then
            # the raw VTune result is several GB and nothing downstream reads it
            [[ ${KEEP_VTUNE:-0} == 1 ]] || rm -rf "vt_$tag"
        else
            echo "WARN: no VTune report for $tag (see vtune.out); raw result kept in vt_$tag" >&2
        fi
    done
}
...
stage_time_intel() {
    ...
    "./$BIN_CU" "$N" "$REPS" ${KERNELS:+"$KERNELS"} > "$NAT.out" 2> "$NAT.err"
    if [[ -x $BIN_OMP ]]; then
