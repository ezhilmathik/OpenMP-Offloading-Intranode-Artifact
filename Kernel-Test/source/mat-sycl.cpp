...
//        ./matmul_sycl <N> [reps=5] [kernels]
//
// "kernels" is an optional comma-separated list selecting which kernels run:
//   naive, naive1d, tiled32, regtile64, regtile128   (default: all)
// e.g. "tiled32" runs only the shared-memory tiled kernel.
//
...
int main(int argc, char **argv)
{
    ...
    auto want = [&](const char *key) {
        ...
        for (const char *p = strstr(sel, key); p; p = strstr(p + 1, key))
            if ((p == sel || p[-1] == ',') && (p[n] == '\0' || p[n] == ','))
                return true;
        return false;
    };
    ...
    auto run = [&](const char *key, const char *name, const char *cfg, auto submit) {
        if (!want(key)) return;
    ...
    run("naive", "naive (32x8 threads)",    cfg[0], [&] { return naive(32, 8); });
    run("naive1d", "naive (128x1, OMP-like)", cfg[1], [&] { return naive(128, 1); });

    // ---- 2. tiled32: 32x32 tiles in local memory ---------------------------
    run("tiled32", "tiled32 (your original)", cfg[2], [&] {
    ...
    run("regtile64", "regtile 64x64x16, 4x4",  cfg[3], [&] {
    ...
    run("regtile128", "regtile 128x128x8, 8x8", cfg[4], [&] {
