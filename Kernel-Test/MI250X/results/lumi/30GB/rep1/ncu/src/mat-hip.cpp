...
//
// All hand-written kernels add the k terms in order 0..N-1, one rounded multiply
// and one rounded add per term. With --fmad=false (CUDA) and -ffp-contract=off
// (host, and Clang for the OpenMP version) the results are therefore bitwise
// identical to a plain sequential CPU triple loop, which is what the check tests.
...
template <class F>
static float time_ms(F f, int reps, hipEvent_t a, hipEvent_t b)
{
    f();
    CUDA_CHECK(hipGetLastError());
    CUDA_CHECK(hipDeviceSynchronize());
    CUDA_CHECK(hipEventRecord(a));
    for (int r = 0; r < reps; ++r) f();
    CUDA_CHECK(hipEventRecord(b));
    CUDA_CHECK(hipEventSynchronize(b));
    CUDA_CHECK(hipGetLastError());
    float ms = 0.0f;
    CUDA_CHECK(hipEventElapsedTime(&ms, a, b));
    return ms / reps;
}
...
{
    if (argc < 2) { fprintf(stderr, "Usage: %s <N> [reps=5] [kernels]\n", argv[0]); return 1; }
    ...
    auto run = [&](const char* key, const char* name, auto launch) {
        if (!want(key)) return;
        CUDA_CHECK(hipMemset(dC, 0xFF, bytes));   // fill with NaN: unwritten entries fail the check
    ...
    run("naive", "naive (32x8 threads)", [&] {
        dim3 block(32, 8), grid((N + 31) / 32, (N + 7) / 8);
        dgemm_naive<<<grid, block>>>(dA, dB, dC, N);
    });
    run("naive1d", "naive (128x1, OMP-like)", [&] {
        dim3 block(128, 1), grid((N + 127) / 128, N);
        dgemm_naive<<<grid, block>>>(dA, dB, dC, N);
    });
    run("tiled32", "tiled32 (your original)", [&] {
        dim3 block(TILE, TILE), grid((N + TILE - 1) / TILE, (N + TILE - 1) / TILE);
        dgemm_tiled32<<<grid, block>>>(dA, dB, dC, N);
    });
    run("regtile64", "regtile 64x64x16, 4x4",  [&] { launch_regtile< 64,  64, 16, 4, 4>(dA, dB, dC, N); });
    run("regtile128", "regtile 128x128x8, 8x8", [&] { launch_regtile<128, 128,  8, 8, 8>(dA, dB, dC, N); });
    ...
    if (hipblasCreate(&handle) != HIPBLAS_STATUS_SUCCESS) { fprintf(stderr, "hipblasCreate failed\n"); return 1; }
    const double one = 1.0, zero = 0.0;
    run("hipblasDgemm (reference)", [&] {
        hipblasDgemm(handle, HIPBLAS_OP_N, HIPBLAS_OP_N, N, N, N, &one, dB, N, dA, N, &zero, dC, N);
    });
