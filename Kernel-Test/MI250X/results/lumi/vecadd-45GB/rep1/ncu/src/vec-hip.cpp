...
//
// Every timed call is launch + wait (hipDeviceSynchronize), exactly like a
...
// that every load instruction of a warp is fully coalesced. hipMalloc returns
...
int main(int argc, char** argv)
{
    ...
    auto run = [&](const char* name, auto launch) {
        CUDA_CHECK(hipMemset(dC, 0xFF, bytes));        // NaN: unwritten entries fail the check
