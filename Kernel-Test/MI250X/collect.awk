# collect.awk -- one dataset (site x size x rep) -> printed table,   [v11: AMD 3-D launch dims]
...
fn == "cuda.out" || fn == "omp.out" || fn == "hip.out" {
    im = ""
...
END {
    ...
    NAT = (("vendor" in META) && META["vendor"] == "amd") ? "HIP" : "CUDA"
    P1  = (NAT == "HIP") ? "rocprof" : "nsys"        # trace / sampled metrics
    P2  = (NAT == "HIP") ? "PMC"     : "ncu"         # counter run
    ...
    if (bench == "matvec") {
        ...
        LBL["cuda_mvt"] = NAT " thread/row"; LBL["cuda_mvw"] = NAT " warp/row"
        ...
        LBL["cuda_vadd"] = NAT " 1-D"; LBL["cuda_v2d"] = NAT " 2-D grid"; LBL["cuda_vvec"] = NAT " double2"
    ...
    for (ri = 1; ri <= nr; ri++) {
        ...
        for (c = 1; c <= nc; c++) {
            ...
            if (row == 12) { name = "L1 / L2 hit % " P2;         v = ncv(im, (L1[im] == "" && L2[im] == "") ? "n/a" : sprintf("%s / %s",
                                   (L1[im] == "") ? "-" : sprintf("%.1f", L1[im]),
                                   (L2[im] == "") ? "-" : sprintf("%.1f", L2[im]))) }
            if (row == 13) { name = (NAT == "HIP") ? "Mem unit stalled % " P2 : "SM throughput % " P2;       v = ncv(im, f1(SMN[im])) }
