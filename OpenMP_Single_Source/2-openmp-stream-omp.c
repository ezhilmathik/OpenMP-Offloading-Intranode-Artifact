...
/* OpenMP target offload implementation of the 3D heat conduction problem
   ...
   This file is 2-openmp-stream.c with one host thread per device. Everything
   outside the time loop is unchanged -- same setup, same allocation, same
   copy-back -- so that a diff shows only what the second host thread adds.

   WHAT IS NEW HERE RELATIVE TO 2-openmp-stream.c
   ----------------------------------------------
   2-openmp-stream.c drives both devices from one host thread: the launches
   for GPU1 cannot be issued until those for GPU0 have been. Here thread tid
   drives device tid, so the two launch sequences proceed independently.

   The dependency machinery gets simpler, not harder. The tokens and the
   depend-objects are declared inside the parallel region, so each thread has
   its own private set and the dependence graph is per-thread. A thread
   cannot wait on the other thread's tasks -- depend clauses order sibling
   tasks, and the two threads' target tasks live in different implicit task
   regions -- so the exchange is arranged as a PUSH to keep every dependence
   inside one thread. See PUSH, NOT PULL below.
   ...
   Two tokens, two objects -- identical to 2-openmp-stream.c, so a diff
   between the two files shows only the threading.
   ...
   PUSH buys. In the pull form thread 0's H2D would read h_halo1_dn -- thread
   1's buffer -- and no token could express it, because depend clauses order
   sibling tasks and the two threads' target tasks live in different implicit
   task regions.
   ...
   WHICH THREAD performs each copy.
   ...
   read h_halo1_dn,
   which thread 1 wrote, and no depend clause can express that; it needed a
   mid-step barrier, and an `omp barrier` completes every explicit task bound
   to the region, including the interior kernel. Push removes the barrier and
   with it the stall.
   ...
   Per timestep, per thread:
     ...
     D2H        async   depend(inout: its halo token)
     H2D        async   depend(inout: its halo token)  -- PUSH into the
                                    neighbour's ghost, from OUR OWN buffer
     taskwait                       -- drains interior, D2H and H2D
...
int main(int argc, char** argv)
{
  ...
  printf("\n**** TWO GPUS OpenMP Offloading, ASYNC + 2 HOST THREADS STARTS ****\n");
  ...
  // Two buffers: one interface, two directions. SHARED between the host
  // threads -- that is what the barriers below protect.
  ...
  //
  // ONE token per halo, exactly as in 2-openmp-stream.c. Each describes a
...
#pragma omp critical
    ...
    // the interval cover both threads' work rather than just thread 0's.
#pragma omp barrier
...
#pragma omp barrier
    ...
    for (int s=0; s<nsteps; s++) {
      ...
      // -- This thread's compute, its D2H, and its PUSH ------------
      // Every operation below touches only data THIS thread produced, so
      // the whole chain is ordered by this thread's own tokens. See PUSH,
      ...
      if (tid == 0) {
        ...
        // D2H: our plane k=1 -> our own buffer. Next link on the _dn chain.
        ...
        // H2D: PUSH our buffer into GPU0's ghost k=nz0-1 -- last link on our
        // own chain, same object as the D2H above.
...
#pragma omp depobj(halo1_dn_dep) destroy
  ...
  if (should_write_txt())
    write_u_values("u_openmp_stream_omp_2gpu.txt", h_u_global, n);
  ...
  printf("\n**** TWO GPUS OpenMP Offloading, ASYNC + 2 HOST THREADS FINISHED ****\n");
  return 0;
}
