#!/usr/bin/env python3
"""
Bar-chart averaging script for HIP vs OpenMP Offloading.
...
aggregate.sh, NOT from the slurm .out files:
    ...
    <PLOTS_DIR>/HIP/summary.csv
    <PLOTS_DIR>/OpenMP/summary.csv

Run this script from the directory that contains the HIP/ and OpenMP/
folders (i.e. AMD-MI250X/), because PLOTS_DIR is ".".
...
The framework (HIP vs OpenMP Offloading) is decided by which folder a
...
# listed here is used verbatim, so "speedup.py plots OpenMP HIP" still labels
...
def build_series_for_gpu(progs_seen, gpu_count):
    ...
    (HIP and OpenMP) so every chart carries the single-GPU reference.
    ...
    # the actual multi-GPU variants, interleaved HIP / OpenMP as before
...
def plot_chart(series, avg, ns, gpu_count, out_pdf, title_suffix, show=True):
    ...
    ax.set_title(f"AMD MI250X - {gpu_count} GPUs {title_suffix}")
    ...
    # variants there are. Two columns per framework keeps HIP/OpenMP aligned.
...
def plot_chart(series, avg, ns, gpu_count, out_pdf, title_suffix, show=True):
    ...
    ax.set_xlabel("Grid Size ($N^3$).", fontsize=MEDIUM_SIZE)
    ax.set_ylabel("Solver Time in Seconds.", fontsize=MEDIUM_SIZE)
    ax.set_title(f"AMD MI250X - {gpu_count} GPUs (1 GCD per module) {title_suffix}")
    ax.set_xticks(x + (n_series - 1) / 2.0 * barWidth)
    ax.set_xticklabels([f"${n}^3$" for n in ns])
...
def parse_args():
    p = argparse.ArgumentParser(description="HIP vs OpenMP Offloading bar charts.")
...
def main():
    ...
    for gpu in args.gpus:
        ...
        plot_chart(series, avg, ns, gpu,
                   out_pdf=os.path.join(outdir, f"Diffusion_MI250X_{gpu}gpu.pdf"),
