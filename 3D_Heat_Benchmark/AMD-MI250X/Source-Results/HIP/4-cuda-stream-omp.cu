...
#pragma omp parallel num_threads(ngpus)
  {
    ...
    while (t < T) {
      ...
      if (tid == 0) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(h_halo1_up,
        ...
        CUDA_CHECK(cudaMemcpyAsync(h_halo2_dn,
                                   d2_u_new + (size_t)1 * ny * nx,
                                   halo_bytes, cudaMemcpyDeviceToHost,
                                   halo_stream_bottom[2]));
        CUDA_CHECK(cudaMemcpyAsync(h_halo2_up,
        ...
        CUDA_CHECK(cudaMemcpyAsync(h_halo3_dn,
...
#pragma omp barrier
      ...
      if (tid == 0) {
        CUDA_CHECK(cudaMemcpyAsync(d0_u_new + (size_t)(nz0-1) * ny * nx,
                                   ...
                                   cudaMemcpyHostToDevice, halo_stream_top[0]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[0]));
        ...
        CUDA_CHECK(cudaMemcpyAsync(d1_u_new + (size_t)0 * ny * nx,
                                   ...
                                   cudaMemcpyHostToDevice, halo_stream_bottom[1]));
        CUDA_CHECK(cudaMemcpyAsync(d1_u_new + (size_t)(nz1-1) * ny * nx,
                                   h_halo2_dn, halo_bytes,
                                   cudaMemcpyHostToDevice, halo_stream_top[1]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[1]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[1]));
        CUDA_CHECK(cudaStreamSynchronize(compute_stream[1]));
      }
      else if (tid == 2) {
        CUDA_CHECK(cudaMemcpyAsync(d2_u_new + (size_t)0 * ny * nx,
                                   h_halo1_up, halo_bytes,
                                   cudaMemcpyHostToDevice, halo_stream_bottom[2]));
        CUDA_CHECK(cudaMemcpyAsync(d2_u_new + (size_t)(nz2-1) * ny * nx,
                                   h_halo3_dn, halo_bytes,
                                   cudaMemcpyHostToDevice, halo_stream_top[2]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[2]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[2]));
        CUDA_CHECK(cudaStreamSynchronize(compute_stream[2]));
      }
      else {
        CUDA_CHECK(cudaMemcpyAsync(d3_u_new + (size_t)0 * ny * nx,
                                   h_halo2_up, halo_bytes,
                                   cudaMemcpyHostToDevice, halo_stream_bottom[3]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[3]));
