...
/* CUDA implementation of the 3D heat conduction problem
   ...
   six hipMemcpyPeerAsync calls replace the six D2H/H2D pairs and the six
...
int main(int argc, char** argv)
{
  ...
  printf("\n**** FOUR GPUS HIP, P2P + 4 HOST THREADS STARTS ****\n");
  ...
  for (int p = 0; p < 3; p++) {
    ...
    CUDA_CHECK(hipDeviceCanAccessPeer(&can_ab, a, b));
    CUDA_CHECK(hipDeviceCanAccessPeer(&can_ba, b, a));
    if (!can_ab || !can_ba) {
      fprintf(stderr, "ERROR: P2P not available between GPU %d and GPU %d; "
                      "this version requires it\n", a, b);
      exit(EXIT_FAILURE);
    }
    CUDA_CHECK(hipSetDevice(a)); CUDA_CHECK(hipDeviceEnablePeerAccess(b, 0));
    CUDA_CHECK(hipSetDevice(b)); CUDA_CHECK(hipDeviceEnablePeerAccess(a, 0));
...
#pragma omp single
  ...
  if (should_write_txt())
    write_u_values("u_hip_stream_omp_p2p_4gpu.txt", h_u_global, n);
  ...
  for (int p = 0; p < 3; p++) {
    int a = p2p_pairs[p][0], b = p2p_pairs[p][1];
    CUDA_CHECK(hipSetDevice(a)); hipDeviceDisablePeerAccess(b);
    CUDA_CHECK(hipSetDevice(b)); hipDeviceDisablePeerAccess(a);
  }
  ...
  printf("\n**** FOUR GPUS HIP, P2P + 4 HOST THREADS FINISHED ****\n");
  return 0;
}
