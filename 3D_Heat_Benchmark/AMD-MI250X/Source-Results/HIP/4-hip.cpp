...
int main(int argc, char** argv)
{
  ...
  while (t < T) {
    ...
    CUDA_CHECK(hipSetDevice(0));
    CUDA_CHECK(hipMemcpy(h_halo0_up, d0_u_new + (size_t)(nz0-2) * ny * nx,
                          halo_bytes, hipMemcpyDeviceToHost));

    CUDA_CHECK(hipSetDevice(1));
    CUDA_CHECK(hipMemcpy(h_halo1_dn, d1_u_new + (size_t)1 * ny * nx,
                          halo_bytes, hipMemcpyDeviceToHost));
    CUDA_CHECK(hipMemcpy(h_halo1_up, d1_u_new + (size_t)(nz1-2) * ny * nx,
                          halo_bytes, hipMemcpyDeviceToHost));

    CUDA_CHECK(hipSetDevice(2));
    CUDA_CHECK(hipMemcpy(h_halo2_dn, d2_u_new + (size_t)1 * ny * nx,
                          halo_bytes, hipMemcpyDeviceToHost));
    CUDA_CHECK(hipMemcpy(h_halo2_up, d2_u_new + (size_t)(nz2-2) * ny * nx,
                          halo_bytes, hipMemcpyDeviceToHost));

    CUDA_CHECK(hipSetDevice(3));
    CUDA_CHECK(hipMemcpy(h_halo3_dn, d3_u_new + (size_t)1 * ny * nx,
                          halo_bytes, hipMemcpyDeviceToHost));

    // -- PHASE 2: H2D into contiguous ghost planes ----------------
    CUDA_CHECK(hipSetDevice(0));
    CUDA_CHECK(hipMemcpy(d0_u_new + (size_t)(nz0-1) * ny * nx, h_halo1_dn,
                          halo_bytes, hipMemcpyHostToDevice));

    CUDA_CHECK(hipSetDevice(1));
    CUDA_CHECK(hipMemcpy(d1_u_new + (size_t)0 * ny * nx, h_halo0_up,
                          halo_bytes, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d1_u_new + (size_t)(nz1-1) * ny * nx, h_halo2_dn,
                          halo_bytes, hipMemcpyHostToDevice));

    CUDA_CHECK(hipSetDevice(2));
    CUDA_CHECK(hipMemcpy(d2_u_new + (size_t)0 * ny * nx, h_halo1_up,
                          halo_bytes, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d2_u_new + (size_t)(nz2-1) * ny * nx, h_halo3_dn,
                          halo_bytes, hipMemcpyHostToDevice));

    CUDA_CHECK(hipSetDevice(3));
    CUDA_CHECK(hipMemcpy(d3_u_new + (size_t)0 * ny * nx, h_halo2_up,
                          halo_bytes, hipMemcpyHostToDevice));
  ...
  if (should_write_txt())
    write_u_values("u_hip_4gpu.txt", h_u_global, n);
