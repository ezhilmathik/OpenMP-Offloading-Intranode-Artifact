...
int main(int argc, char** argv)
{
  ...
  while (t < T) {
    ...
    CUDA_CHECK(hipSetDevice(0));
    CUDA_CHECK(hipMemcpyAsync(h_halo0_up, d0_u_new + (size_t)(nz0-2) * ny * nx,
                               halo_bytes, hipMemcpyDeviceToHost,
                               halo_stream_top[0]));

    CUDA_CHECK(hipSetDevice(1));
    CUDA_CHECK(hipMemcpyAsync(h_halo1_dn, d1_u_new + (size_t)1 * ny * nx,
                               halo_bytes, hipMemcpyDeviceToHost,
                               halo_stream_bottom[1]));
    CUDA_CHECK(hipMemcpyAsync(h_halo1_up, d1_u_new + (size_t)(nz1-2) * ny * nx,
                               halo_bytes, hipMemcpyDeviceToHost,
                               halo_stream_top[1]));

    CUDA_CHECK(hipSetDevice(2));
    CUDA_CHECK(hipMemcpyAsync(h_halo2_dn, d2_u_new + (size_t)1 * ny * nx,
                               halo_bytes, hipMemcpyDeviceToHost,
                               halo_stream_bottom[2]));
    CUDA_CHECK(hipMemcpyAsync(h_halo2_up, d2_u_new + (size_t)(nz2-2) * ny * nx,
                               halo_bytes, hipMemcpyDeviceToHost,
                               halo_stream_top[2]));

    CUDA_CHECK(hipSetDevice(3));
    CUDA_CHECK(hipMemcpyAsync(h_halo3_dn, d3_u_new + (size_t)1 * ny * nx,
                               halo_bytes, hipMemcpyDeviceToHost,
    ...
    CUDA_CHECK(hipSetDevice(0));
    CUDA_CHECK(hipMemcpyAsync(d0_u_new + (size_t)(nz0-1) * ny * nx, h_halo1_dn,
                               halo_bytes, hipMemcpyHostToDevice,
                               halo_stream_top[0]));

    CUDA_CHECK(hipSetDevice(1));
    CUDA_CHECK(hipMemcpyAsync(d1_u_new + (size_t)0 * ny * nx, h_halo0_up,
                               halo_bytes, hipMemcpyHostToDevice,
                               halo_stream_bottom[1]));
    CUDA_CHECK(hipMemcpyAsync(d1_u_new + (size_t)(nz1-1) * ny * nx, h_halo2_dn,
                               halo_bytes, hipMemcpyHostToDevice,
                               halo_stream_top[1]));

    CUDA_CHECK(hipSetDevice(2));
    CUDA_CHECK(hipMemcpyAsync(d2_u_new + (size_t)0 * ny * nx, h_halo1_up,
                               halo_bytes, hipMemcpyHostToDevice,
                               halo_stream_bottom[2]));
    CUDA_CHECK(hipMemcpyAsync(d2_u_new + (size_t)(nz2-1) * ny * nx, h_halo3_dn,
                               halo_bytes, hipMemcpyHostToDevice,
                               halo_stream_top[2]));

    CUDA_CHECK(hipSetDevice(3));
    CUDA_CHECK(hipMemcpyAsync(d3_u_new + (size_t)0 * ny * nx, h_halo2_up,
                               halo_bytes, hipMemcpyHostToDevice,
                               halo_stream_bottom[3]));
    ...
    for (int g = 0; g < ngpus; ++g) {
      CUDA_CHECK(hipSetDevice(g));
      CUDA_CHECK(hipStreamSynchronize(compute_stream[g]));
      CUDA_CHECK(hipStreamSynchronize(halo_stream_bottom[g]));
      CUDA_CHECK(hipStreamSynchronize(halo_stream_top[g]));
    }
  ...
  if (should_write_txt())
    write_u_values("u_hip_stream_4gpu.txt", h_u_global, n);
