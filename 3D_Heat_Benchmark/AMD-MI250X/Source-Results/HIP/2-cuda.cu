...
int main(int argc, char** argv)
{
  ...
  while (t < T) {
    ...
    CUDA_CHECK(cudaSetDevice(0));
    CUDA_CHECK(cudaMemcpy(h_halo0_up,
                          d0_u_new + (size_t)(nz0-2) * ny * nx,
                          halo_bytes, cudaMemcpyDeviceToHost));

    CUDA_CHECK(cudaSetDevice(1));
    CUDA_CHECK(cudaMemcpy(h_halo1_dn,
                          d1_u_new + (size_t)1 * ny * nx,
                          halo_bytes, cudaMemcpyDeviceToHost));

    // -- PHASE 2: H2D into contiguous ghost planes ----------------
    CUDA_CHECK(cudaSetDevice(0));
    CUDA_CHECK(cudaMemcpy(d0_u_new + (size_t)(nz0-1) * ny * nx,
                          h_halo1_dn,
                          halo_bytes, cudaMemcpyHostToDevice));

    CUDA_CHECK(cudaSetDevice(1));
    CUDA_CHECK(cudaMemcpy(d1_u_new + (size_t)0 * ny * nx,
                          h_halo0_up,
                          halo_bytes, cudaMemcpyHostToDevice));
