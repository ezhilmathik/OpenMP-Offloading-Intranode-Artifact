...
/* CUDA implementation of the 3D heat conduction problem
   ...
   --------------------------------------------
   An earlier version of this file used hipHostMalloc (pinned), while
...
int main(int argc, char** argv)
{
  ...
  printf("\n**** TWO GPUS HIP, ASYNC + 2 HOST THREADS STARTS ****\n");
...
#pragma omp single
  ...
  if (should_write_txt())
    write_u_values("u_hip_stream_omp_2gpu.txt", h_u_global, n);
  ...
  printf("\n**** TWO GPUS HIP, ASYNC + 2 HOST THREADS FINISHED ****\n");
  return 0;
}
