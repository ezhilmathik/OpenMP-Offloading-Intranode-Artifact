...
/* CUDA implementation of the 3D heat conduction problem
   ...
   and no pack/unpack kernels are needed:
     D2H: hipMemcpyAsync(h_halo, d_u_new + k*(ny*nx), halo_bytes, D2H, s)
     H2D: hipMemcpyAsync(d_u_new + ghost_k*(ny*nx), h_halo, halo_bytes, H2D, s)
   Neighbour offsets:
     i+-1  ->  +-1
     j+-1  ->  +-nx
     k+-1  ->  +-ny*nx
   ...
   --------------------------------------------
   hipHostAlloc'd pinned buffers would make these copies genuinely
...
int main(int argc, char** argv)
{
  ...
  while (t < T) {
    ...
    CUDA_CHECK(hipMemcpyAsync(d0_u_new + (size_t)(nz0-1) * ny * nx,
                               ...
                               hipMemcpyHostToDevice, halo_stream[0]));

    CUDA_CHECK(hipSetDevice(1));
    CUDA_CHECK(hipMemcpyAsync(d1_u_new + (size_t)0 * ny * nx,
                               h_halo0_up, halo_bytes,
                               hipMemcpyHostToDevice, halo_stream[1]));
  ...
  if (should_write_txt())
    write_u_values("u_hip_stream_2gpu.txt", h_u_global, n);
