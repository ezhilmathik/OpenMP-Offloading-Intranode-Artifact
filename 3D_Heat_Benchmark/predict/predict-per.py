...
predicted time per step with the signed deviation. No fitting is done here:
...
def main():
    ...
    results = []   # (label, n, P, meas_ms, pred_ms, dev_pct, spread, samples)
    for key in machines:
        m = MACHINES[key]
        ...
        for (P, n), (t, steps, spread, samples) in sorted(rows.items(),
            ...
            meas = t / steps
            pred = predict_step(n, P, m["gamma"], m["beta"], args.alpha)
            dev = 100.0 * (pred - meas) / meas
            results.append((m["label"], n, P, 1e3 * meas, 1e3 * pred, dev, spread, samples))

    if not results:
        sys.exit("nothing to report")

    if args.latex:
        print("% GPU & N & GPUs & meas (ms/step) & spread (%) & pred (ms/step) & dev (%)")
        for label, n, P, meas, pred, dev, spread, _ in results:
            sign = f"{dev:+.1f}" if abs(dev) >= 0.05 else r"\phantom{+}0.0"
            print(f"{label:<9} & ${n}^3$ & {P} & {meas:6.1f} & {spread:4.1f} & "
                  f"{pred:6.1f} & ${sign}$ \\\\")
    else:
        hdr = f"{'GPU':<9} {'N':>6} {'GPUs':>4} {'meas ms/step':>13} " \
              f"{'spread%':>8} {'pred ms/step':>13} {'dev%':>7} {'n':>3}"
        print(hdr)
        print("-" * len(hdr))
        for label, n, P, meas, pred, dev, spread, samples in results:
            print(f"{label:<9} {n:>5}^3 {P:>4} {meas:>13.1f} {spread:>8.1f} "
                  f"{pred:>13.1f} {dev:>+7.1f} {samples:>3}")
