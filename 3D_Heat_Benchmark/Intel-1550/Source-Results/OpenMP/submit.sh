...
# for every grid size. Intel Data Center GPU Max 1550 (PVC).
# ---------------------------------------------------------------------------
#
...
#    HIER       COMPOSITE (1 device = whole card) | FLAT (1 device = 1 stack)
#    FAMILIES   "2 4" | "2" | "4"
...
if [[ "$HIER" == "COMPOSITE" ]]; then
    UNIT="1 device = 1 whole Max 1550 card (2 stacks)"
else
    UNIT="1 device = 1 stack; masks pick one stack per card (0,2,4,6)"
fi

echo "============================================================"
echo "Submitting 3D-heat timing pipeline  (Intel PVC Max 1550)"
echo "============================================================"
echo "  Repetitions : $N_RUNS"
echo "  Grid sizes  : $NS"
echo "  Hierarchy   : $HIER  -- $UNIT"
...
if [[ ${#MISSING[@]} -gt 0 ]]; then
    ...
    echo "       Check build-*.out -- not submitting the sweep." >&2
    exit 1
fi
