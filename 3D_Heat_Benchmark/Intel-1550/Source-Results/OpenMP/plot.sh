...
bars() {
    ...
    END {
        ...
        printf "\n  Speedup relative to 1-omp at the same N.\n" }' "$SUMMARY"
}
...
group() {
    ...
    function rank(x) {
        if (x == "1-omp")                   return 1
        if (x == fam "-omp")                return 2
        if (x == fam "-omp-stream")         return 3
        if (x == fam "-omp-stream-omp")     return 4
        if (x == fam "-omp-stream-omp-p2p") return 5
        return 0 }
    NR == 1 { next }
    $cm == "N/A" { next }
...
speedup() {
    ...
    $cm == "N/A" { next }
    { n[NR]=$cn+0; v[NR]=$cv; g[NR]=$cg; t[NR]=$cm+0; rows=NR
      if (v[NR] == "1-omp") base[n[NR]] = t[NR]
      seenN[$cn+0] = 1 }
    END {
        printf "\n  Speedup vs 1-omp  (ideal: 2.00x on 2 GPUs, 4.00x on 4)\n\n"
        cnt = 0
        ...
        for (o = 1; o <= cnt; o++) { N = ord[o]
            ...
            printf "  N=%d\n", N
            for (i = 1; i <= rows; i++) if (n[i] == N && v[i] != "1-omp" && t[i] > 0) {
                sp = base[N] / t[i]
