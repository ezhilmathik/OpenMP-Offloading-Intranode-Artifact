...
int main(int argc, char **argv) try {
  ...
  // Build the one context that spans all 2 devices. Must precede every
  ...
  // FALSE NEGATIVE on oneAPI 2024.1.0 / Max 1550. ext_oneapi_can_access_peer()
  // reports no peer access and ext_oneapi_enable_peer_access() throws
  // PI_ERROR_INVALID_OPERATION (-59), yet the cross-device copy in a shared
  // context measures 62.9 GB/s against 25.3 GB/s for the same copy staged
  // through host memory (probe_p2p, 64 MB, COMPOSITE), and xpu-smi topology
  // shows XL8/XL* Xe Link between every card pair. The capability is present;
  // only the query is broken. Warn, do not abort.
  //
  // Calibrated on those numbers, not a blanket relaxation. Re-run probe_p2p
  // after any toolkit or driver change: if the direct and staged rates
  // converge, this variant is host-staged wearing a p2p label and this guard
  // must be fatal again.
  if (!can01 || !can10) {
    fprintf(stderr, "WARNING: can_access_peer reports no P2P (GPU 0 <-> GPU 1); "
...
#pragma omp single
  ...
  */
  ...
  if (should_write_txt())
    write_u_values("u_sycl_stream_omp_p2p_2gpu.txt", h_u_global, n);
  ...
  for (int g = 0; g < ngpus; ++g) {
    ...
    */
    ...
    CUDA_CHECK(DPCT_CHECK_ERROR(
        p2p_destroy_queue(halo_stream[g])));
  }
  ...
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
      CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_current_device().ext_oneapi_disable_peer_access(dpct::dev_mgr::instance().get_device(1))));  // wrapped: disable throws -59 on this toolkit, and an unwrapped throw here aborts AFTER the result is already written
  /*
  DPCT1093:147: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
      CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_current_device().ext_oneapi_disable_peer_access(dpct::dev_mgr::instance().get_device(0))));  // wrapped: disable throws -59 on this toolkit, and an unwrapped throw here aborts AFTER the result is already written
