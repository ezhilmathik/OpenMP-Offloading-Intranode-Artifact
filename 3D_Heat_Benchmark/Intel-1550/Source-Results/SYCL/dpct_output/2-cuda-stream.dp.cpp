...
/* CUDA implementation of the 3D heat conduction problem
   ...
   TWO-GPU VERSION, ASYNC. Halo exchange staged through the host on a
   dedicated stream, overlapped with the interior kernel.
   ...
   and no pack/unpack kernels are needed:
     D2H: cudaMemcpyAsync(h_halo, d_u_new + k*(ny*nx), halo_bytes, D2H, s)
     H2D: cudaMemcpyAsync(d_u_new + ghost_k*(ny*nx), h_halo, halo_bytes, H2D, s)
   Neighbour offsets:
     i+-1  ->  +-1
     j+-1  ->  +-nx
     k+-1  ->  +-ny*nx

   This file is kept deliberately parallel to 2-openmp-stream.c and differs
   from 2-cuda.cu only in the asynchrony: same decomposition, same kernel
   split, same launch order, same prints, same timing.
   ...
   Here the split does real work: the boundary kernel runs on halo_stream so
   the exchanged plane finishes early and its D2H can be issued while the
   interior kernel is still running on compute_stream. A plane that is never
   sent has no such urgency, so folding it into the interior is strictly
   better -- it keeps the boundary kernel, which gates the exchange, as small
   as possible.
   ...
   STREAM ASSIGNMENT
   -----------------
     compute_stream[g]   interior kernel
     halo_stream[g]      boundary kernel, then D2H, then H2D

   Boundary -> D2H -> H2D are all on halo_stream, so their order is
   guaranteed without any explicit event. The interior kernel is launched
   FIRST (matching 2-openmp-stream.c) so it is resident while the exchange
   proceeds beside it.

   HOST HALO BUFFERS ARE PAGEABLE, DELIBERATELY
   --------------------------------------------
   cudaHostAlloc'd pinned buffers would make these copies genuinely
   asynchronous and would very likely be faster. They are NOT used because
   2-openmp-stream.c stages through plain malloc'd host memory, and the point
   of this file is to measure the same algorithm under CUDA rather than a
   better-tuned one. Switching to pinned memory here would make the CUDA
   numbers incomparable to the OpenMP numbers.
   ...
   BUILD NOTE: two flags are mandatory for a valid comparison.
     ...
     nvcc -O3 -arch=sm_90 --fmad=false -Xcompiler -fopenmp 2-cuda-stream.cu -lm -o 2-cuda-stream
*/
...
/*
DPCT1009:79: SYCL uses exceptions to report errors and does not use the error
...
// Identical to 2-openmp-stream.c. Boundary values are forced to exact zero.
...
int main(int argc, char **argv) try {
  ...
  */
  ...
  /*
  DPCT1093:81: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  ...
  for (int g = 0; g < ngpus; ++g) {
    /*
    DPCT1093:82: The "g" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
  ...
  while (t < T) {
    ...
    // -- GPU 0: interior first, then top boundary on halo_stream --
    /*
    ...
    */
    ...
    // -- GPU 1: interior first, then bottom boundary on halo_stream
    /*
    ...
    */
    ...
    /*
    DPCT1093:89: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
    /*
    DPCT1124:90: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    ...
    */
    ...
    /*
    DPCT1093:91: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
    /*
    DPCT1124:92: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    // -- Both D2H must land before either H2D reads the host buffer

    /*
    DPCT1093:93: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
    CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream[0]->wait()));
    /*
    DPCT1093:94: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
    ...
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
    /*
    DPCT1124:96: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1093:97: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
    /*
    DPCT1124:98: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    // -- Drain both streams on both devices before the swap -------
    /*
    DPCT1093:99: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
    CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream[0]->wait()));
    ...
    /*
    DPCT1093:100: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
    CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream[1]->wait()));
  ...
  // idle. Make it explicit so the timed region cannot drift if the final
  ...
  */
  ...
  /*
  DPCT1093:104: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
  ...
  if (should_write_txt())
    write_u_values("u_cuda_stream_2gpu.txt", h_u_global, n);

  // -- Cleanup streams -------------------------------------------
  for (int g = 0; g < ngpus; ++g) {
    /*
    DPCT1093:105: The "g" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(g)));
    CUDA_CHECK(DPCT_CHECK_ERROR(
  ...
  // -- Cleanup ---------------------------------------------------
  /*
  DPCT1093:106: The "0" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
  CUDA_CHECK(
  ...
  /*
  DPCT1093:107: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
  CUDA_CHECK(
