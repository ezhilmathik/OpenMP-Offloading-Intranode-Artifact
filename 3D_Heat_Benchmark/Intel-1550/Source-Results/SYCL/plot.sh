...
#   ./plot.sh -s              speedup vs 1-sycl, all grid sizes
#   ./plot.sh -a              all of the above
#
#   SUMMARY=other.csv ./plot.sh
#
# Column positions are read from the HEADER, so this works with any of the
# schemas the aggregate scripts emit.
# ---------------------------------------------------------------------------
set -euo pipefail
...
bars() {
    awk -F, -v want="$1" -v w="$WIDTH" -v base_name="$BASE" \
            -v cv="$CV" -v cg="$CG" -v cn="$CN" -v cm="$CM" '
    NR == 1 { next }
    $cm == "N/A" { next }
    ...
    END {
        ...
        for (i = 1; i <= rows; i++) if (keep[i]) {
            ...
            printf "  %-24s %-3s %-*s %8.1fs  %4.2fx\n", v[i], g[i], w, bar, t[i], sp }
        printf "\n  Speedup relative to %s at the same N.\n", base_name }' "$SUMMARY"
}

group() {
    awk -F, -v fam="$1" -v w="$WIDTH" -v base_name="$BASE" \
            -v cv="$CV" -v cg="$CG" -v cn="$CN" -v cm="$CM" '
    function rank(x) {
        if (x == base_name)                  return 1
        if (x == fam "-sycl")                return 2
        if (x == fam "-sycl-stream")         return 3
        if (x == fam "-sycl-stream-omp")     return 4
        if (x == fam "-sycl-stream-omp-p2p") return 5
        return 0 }
    NR == 1 { next }
    $cm == "N/A" { next }
    ...
    END {
        ...
        for (o = 1; o <= cnt; o++) {
            ...
            for (r = 1; r <= 5; r++) {
                ...
                printf "    %-24s %-3s %-*s %8.1fs %5.2fx%s\n", \
                       name[r], gp[r], w, bar, t[N,r], sp, (r == bestr ? "  <= best" : "") }
            if ((N,4) in t && (N,5) in t && t[N,4] > 0) {
                d = 100.0 * (t[N,4] - t[N,5]) / t[N,4]
                printf "    %-24s p2p vs stream-omp: %+.1f%% %s\n", "", d,
                       (d > 1 ? "(p2p faster)" : (d < -1 ? "(p2p SLOWER)" : "(no difference)")) } }
        printf "\n" }' "$SUMMARY"
}

speedup() {
    awk -F, -v w="$WIDTH" -v base_name="$BASE" \
            -v cv="$CV" -v cg="$CG" -v cn="$CN" -v cm="$CM" '
    NR == 1 { next }
    $cm == "N/A" { next }
    { n[NR]=$cn+0; v[NR]=$cv; g[NR]=$cg; t[NR]=$cm+0; rows=NR
      if (v[NR] == base_name) base[n[NR]] = t[NR]
      seenN[$cn+0] = 1 }
    END {
        printf "\n  Speedup vs %s  (ideal: 2.00x on 2 GPUs, 4.00x on 4)\n\n", base_name
        cnt = 0
        ...
        for (o = 1; o <= cnt; o++) { N = ord[o]
            ...
            printf "  N=%d\n", N
            for (i = 1; i <= rows; i++) if (n[i] == N && v[i] != base_name && t[i] > 0) {
                sp = base[N] / t[i]
                len = int(20 * sp); if (len < 1) len = 1; if (len > w) len = w
                bar = ""; for (k = 0; k < len; k++) bar = bar "="
                printf "    %-24s %-3s %-*s %5.2fx\n", v[i], g[i], w, bar, sp }
            print "" } }' "$SUMMARY"
}
