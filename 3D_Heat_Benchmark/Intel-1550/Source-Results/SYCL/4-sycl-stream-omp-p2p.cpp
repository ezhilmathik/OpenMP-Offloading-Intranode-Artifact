...
/* CUDA implementation of the 3D heat conduction problem
   ...
   FOUR-GPU VERSION, P2P + 4 HOST THREADS. Halo exchange goes DIRECTLY
   ...
   This file is kept deliberately parallel to 4-openmp-stream-omp-p2p.c.
   Against 4-cuda-stream-omp.cu the ONLY difference is the transfer path:
   six cudaMemcpyPeerAsync calls replace the six D2H/H2D pairs and the six
   host halo buffers disappear.
   ...
   PULL PATTERN, AND WHY IT DOES NOT RACE
   --------------------------------------
   Every device PULLS into its own ghost planes, so each device writes only
   its OWN memory:

     GPU0 ghost k=nz0-1  <-  GPU1 plane k=1
     GPU1 ghost k=0      <-  GPU0 plane k=nz0-2
     GPU1 ghost k=nz1-1  <-  GPU2 plane k=1
     GPU2 ghost k=0      <-  GPU1 plane k=nz1-2
     GPU2 ghost k=nz2-1  <-  GPU3 plane k=1
     GPU3 ghost k=0      <-  GPU2 plane k=nz2-2

   Three independent reasons the concurrent copies are safe:

     1. On any device the plane being READ by a neighbour (1 or nz-2) is
        never the plane being WRITTEN locally (0 or nz-1). With nz >= 4 those
        four indices are distinct.
     2. The barrier after PHASE 1 guarantees every sent plane is final before
        any pull is issued.
     3. The interior kernel runs concurrently on compute_stream but READS
        u_old and WRITES u_new only in k_start..k_end -- it neither reads nor
        writes any plane the peer copies touch.

   STREAM ASSIGNMENT
   -----------------
     compute_stream[d]        interior kernel
     halo_stream_bottom[d]    bottom boundary kernel, then the pull into
                              ghost k=0            (the LOWER interface)
     halo_stream_top[d]       top boundary kernel, then the pull into
                              ghost k=nz-1         (the UPPER interface)

   NO HOST HALO BUFFERS
   --------------------
   The other files in this set stage through plain malloc'd host memory
   (never pinned -- see 2-cuda-stream.cu's header). This file has no host
   staging at all, which is the entire point of the variant.

   BUILD NOTE:
     nvcc -O3 -arch=sm_90 --fmad=false -Xcompiler -fopenmp 4-cuda-stream-omp-p2p.cu -lm -o 4-cuda-stream-omp-p2p
...
#include <math.h>
#include <omp.h>
#include "p2p_shim.hpp"

#define IDX(k, j, i, ny, nx)  ((size_t)(k)*(size_t)(ny)*(size_t)(nx) \
                             + (size_t)(j)*(size_t)(nx) + (size_t)(i))

/*
DPCT1009:280: SYCL uses exceptions to report errors and does not use the error
...
// Identical to 4-openmp-stream-omp-p2p.c. Boundaries forced to exact zero.
...
// Computes k=1, the plane pulled by the LOWER neighbour. Used by GPU 1, 2, 3.
...
// Computes k=nz-2, the plane pulled by the UPPER neighbour. Used by GPU 0,1,2.
SYCL_EXTERNAL
...
int main(int argc, char **argv) try {
  ...
  printf("\n**** FOUR GPUS CUDA, P2P + 4 HOST THREADS STARTS ****\n");
  ...
  // Build the one context that spans all 4 devices. Must precede every
  // sycl::malloc_device below: that is what makes the peer copies legal.
  p2p_init(4);

  // -- Enable P2P on the three interfaces -----------------------
  // This variant has no host fallback: if any neighbouring pair is not
  // peer-accessible the run is aborted rather than silently routed through
  // the host, which would make the measurement meaningless.
  int p2p_pairs[3][2] = { {0,1}, {1,2}, {2,3} };
  for (int p = 0; p < 3; p++) {
    int a = p2p_pairs[p][0], b = p2p_pairs[p][1];
    int can_ab = 0, can_ba = 0;
    CUDA_CHECK(DPCT_CHECK_ERROR(
        can_ab =
            dpct::dev_mgr::instance().get_device(a).ext_oneapi_can_access_peer(
                dpct::dev_mgr::instance().get_device(b))));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        can_ba =
            dpct::dev_mgr::instance().get_device(b).ext_oneapi_can_access_peer(
                dpct::dev_mgr::instance().get_device(a))));
    // FALSE NEGATIVE on oneAPI 2024.1.0 / Max 1550 -- see the note in
    // 2-sycl-stream-omp-p2p.cpp. Measured 62.9 GB/s direct vs 25.3 GB/s
    // host-staged, with Xe Link present between every card pair.
    if (!can_ab || !can_ba) {
      fprintf(stderr, "WARNING: can_access_peer reports no P2P (GPU %d <-> GPU %d); "
                      "known false negative on this toolkit, proceeding.\n", a, b);
    }
    /*
    DPCT1093:281: The "a" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(a)));
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_current_device().ext_oneapi_enable_peer_access(
                dpct::dev_mgr::instance().get_device(b))));
    /*
    DPCT1093:282: The "b" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(b)));
        CUDA_CHECK(DPCT_CHECK_ERROR(
            dpct::get_current_device().ext_oneapi_enable_peer_access(
                dpct::dev_mgr::instance().get_device(a))));
  ...
  // The boundary kernels write k=1 and k=nz-2 while the peer copies write
  // k=0 and k=nz-1; those four indices must be distinct, which needs nz>=4.
  ...
  /*
  DPCT1093:283: The "0" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  ...
  CUDA_CHECK(DPCT_CHECK_ERROR(d0_rhs = (double *)sycl::malloc_device(
      ...
      CUDA_CHECK(DPCT_CHECK_ERROR(d0_kappa = (double *)sycl::malloc_device(
                                      size0, q_cur())));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d0_u_old, h_u0, size0).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d0_u_new, h_u0, size0).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d0_rhs, h_rhs0, size0).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d0_kappa, h_kappa0, size0).wait()));

  /*
  DPCT1093:284: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  ...
  CUDA_CHECK(DPCT_CHECK_ERROR(d1_rhs = (double *)sycl::malloc_device(
      ...
      CUDA_CHECK(DPCT_CHECK_ERROR(d1_kappa = (double *)sycl::malloc_device(
                                      size1, q_cur())));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d1_u_old, h_u1, size1).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d1_u_new, h_u1, size1).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d1_rhs, h_rhs1, size1).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d1_kappa, h_kappa1, size1).wait()));

  /*
  DPCT1093:285: The "2" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  ...
  CUDA_CHECK(DPCT_CHECK_ERROR(d2_rhs = (double *)sycl::malloc_device(
      ...
      CUDA_CHECK(DPCT_CHECK_ERROR(d2_kappa = (double *)sycl::malloc_device(
                                      size2, q_cur())));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d2_u_old, h_u2, size2).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d2_u_new, h_u2, size2).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d2_rhs, h_rhs2, size2).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d2_kappa, h_kappa2, size2).wait()));

  /*
  DPCT1093:286: The "3" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  ...
  CUDA_CHECK(DPCT_CHECK_ERROR(d3_rhs = (double *)sycl::malloc_device(
      ...
      CUDA_CHECK(DPCT_CHECK_ERROR(d3_kappa = (double *)sycl::malloc_device(
                                      size3, q_cur())));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d3_u_old, h_u3, size3).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d3_u_new, h_u3, size3).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d3_rhs, h_rhs3, size3).wait()));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(d3_kappa, h_kappa3, size3).wait()));
  ...
  for (int g = 0; g < ngpus; ++g) {
    /*
    DPCT1093:287: The "g" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
  ...
  //
  // PHASE 1: interior on compute_stream, boundary kernels on their interface
  //   streams, then each thread synchronises its OWN halo streams so the
  //   planes it exposes are final.
  //
  //   barrier: every sent plane must be final before any peer copy reads it.
  //
  // PHASE 2: each device PULLS into its own ghost planes -- no host.
  //
...
#pragma omp parallel num_threads(ngpus)
  {
    ...
    /*
    DPCT1093:288: The "tid" device may be not the one intended for use. Adjust
    ...
    */
    ...
    while (t < T) {
      ...
      // -- PHASE 1: compute ---------------------------------------
      if (tid == 0) {
        // GPU0: no bottom boundary -- k=1 is never pulled from here.
        /*
        ...
        */
        ...
        // planes k=1 and k=nz1-2 must be final before the neighbours pull
        ...
        */
        ...
        // GPU3: no top boundary -- k=nz3-2 is never pulled from here.
        /*
        ...
        */
        ...
        /*
        DPCT1010:297: SYCL uses exceptions to report errors and does not use the
        error codes. The call was replaced with 0. You need to rewrite this
        code.
        */
        CUDA_CHECK(0);
        ...
        */
        ...
        /*
        DPCT1010:298: SYCL uses exceptions to report errors and does not use the
        error codes. The call was replaced with 0. You need to rewrite this
        code.
        */
        CUDA_CHECK(0);

        // plane k=1 must be final before GPU2 pulls it
        CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_bottom[3]->wait()));
      }

      // Every sent plane is final -- safe to peer-copy.
#pragma omp barrier

      // -- PHASE 2: pull into our own ghost planes ----------------
      if (tid == 0) {
        /*
        DPCT1007:299: Migration of cudaMemcpyPeerAsync is not supported.
        */
        CUDA_CHECK(cudaMemcpyPeerAsync(
            d0_u_new + (size_t)(nz0 - 1) * ny * nx, 0, // dst: GPU0 top ghost
            d1_u_new + (size_t)1 * ny * nx, 1,         // src: GPU1 plane k=1
            halo_bytes, halo_stream_top[0]));
        CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_top[0]->wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(compute_stream[0]->wait()));
      }
      else if (tid == 1) {
        /*
        DPCT1007:300: Migration of cudaMemcpyPeerAsync is not supported.
        */
        CUDA_CHECK(cudaMemcpyPeerAsync(d1_u_new + (size_t)0 * ny * nx,
                                       ...
                                       halo_bytes, halo_stream_bottom[1]));
        /*
        DPCT1007:301: Migration of cudaMemcpyPeerAsync is not supported.
        */
        CUDA_CHECK(cudaMemcpyPeerAsync(
            d1_u_new + (size_t)(nz1 - 1) * ny * nx, 1, // dst: GPU1 top ghost
            d2_u_new + (size_t)1 * ny * nx, 2,         // src: GPU2 plane k=1
            halo_bytes, halo_stream_top[1]));
        CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_bottom[1]->wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_top[1]->wait()));
        ...
        /*
        DPCT1007:302: Migration of cudaMemcpyPeerAsync is not supported.
        */
        CUDA_CHECK(cudaMemcpyPeerAsync(d2_u_new + (size_t)0 * ny * nx,
                                       2, // dst: GPU2 bottom ghost
                                       d1_u_new + (size_t)(nz1 - 2) * ny * nx,
                                       1, // src: GPU1 plane k=nz1-2
                                       halo_bytes, halo_stream_bottom[2]));
        /*
        DPCT1007:303: Migration of cudaMemcpyPeerAsync is not supported.
        */
        CUDA_CHECK(cudaMemcpyPeerAsync(
            d2_u_new + (size_t)(nz2 - 1) * ny * nx, 2, // dst: GPU2 top ghost
            d3_u_new + (size_t)1 * ny * nx, 3,         // src: GPU3 plane k=1
            halo_bytes, halo_stream_top[2]));
        CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_bottom[2]->wait()));
        CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_top[2]->wait()));
        ...
        /*
        DPCT1007:304: Migration of cudaMemcpyPeerAsync is not supported.
        */
        CUDA_CHECK(cudaMemcpyPeerAsync(d3_u_new + (size_t)0 * ny * nx,
                                       3, // dst: GPU3 bottom ghost
                                       d2_u_new + (size_t)(nz2 - 2) * ny * nx,
                                       2, // src: GPU2 plane k=nz2-2
                                       halo_bytes, halo_stream_bottom[3]));
        CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_bottom[3]->wait()));
...
#pragma omp single
  ...
  for (int g = 0; g < ngpus; ++g) {
    /*
    DPCT1093:305: The "g" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(g)));
    CUDA_CHECK(
        DPCT_CHECK_ERROR(p2p_wait_current()));
  }
  ...
  /*
  DPCT1093:306: The "0" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(h_u0, d0_u_old, size0).wait()));
  /*
  DPCT1093:307: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(h_u1, d1_u_old, size1).wait()));
  /*
  DPCT1093:308: The "2" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(2)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(h_u2, d2_u_old, size2).wait()));
  /*
  DPCT1093:309: The "3" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(3)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
      q_cur().memcpy(h_u3, d3_u_old, size3).wait()));
  ...
  if (should_write_txt())
    write_u_values("u_sycl_stream_omp_p2p_4gpu.txt", h_u_global, n);

  // -- Cleanup streams -------------------------------------------
  for (int g = 0; g < ngpus; ++g) {
    /*
    DPCT1093:310: The "g" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(g)));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        p2p_destroy_queue(compute_stream[g])));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        p2p_destroy_queue(halo_stream_bottom[g])));
    CUDA_CHECK(DPCT_CHECK_ERROR(
        p2p_destroy_queue(halo_stream_top[g])));
  }
  ...
  for (int p = 0; p < 3; p++) {
    int a = p2p_pairs[p][0], b = p2p_pairs[p][1];
    /*
    DPCT1093:311: The "a" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(a)));
        CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_current_device().ext_oneapi_disable_peer_access(dpct::dev_mgr::instance().get_device(b))));  // wrapped: disable throws -59 on this toolkit, and an unwrapped throw here aborts AFTER the result is already written
    /*
    DPCT1093:312: The "b" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(b)));
        CUDA_CHECK(DPCT_CHECK_ERROR(dpct::get_current_device().ext_oneapi_disable_peer_access(dpct::dev_mgr::instance().get_device(a))));  // wrapped: disable throws -59 on this toolkit, and an unwrapped throw here aborts AFTER the result is already written
  }

  // -- Cleanup ---------------------------------------------------
  /*
  DPCT1093:313: The "0" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
  CUDA_CHECK(
      DPCT_CHECK_ERROR(dpct::dpct_free(d0_u_old, q_cur())));
      CUDA_CHECK(DPCT_CHECK_ERROR(
          dpct::dpct_free(d0_u_new, q_cur())));
  CUDA_CHECK(
      DPCT_CHECK_ERROR(dpct::dpct_free(d0_rhs, q_cur())));
      CUDA_CHECK(DPCT_CHECK_ERROR(
          dpct::dpct_free(d0_kappa, q_cur())));

  /*
  DPCT1093:314: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
  CUDA_CHECK(
      DPCT_CHECK_ERROR(dpct::dpct_free(d1_u_old, q_cur())));
      CUDA_CHECK(DPCT_CHECK_ERROR(
          dpct::dpct_free(d1_u_new, q_cur())));
  CUDA_CHECK(
      DPCT_CHECK_ERROR(dpct::dpct_free(d1_rhs, q_cur())));
      CUDA_CHECK(DPCT_CHECK_ERROR(
          dpct::dpct_free(d1_kappa, q_cur())));

  /*
  DPCT1093:315: The "2" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(2)));
  CUDA_CHECK(
      DPCT_CHECK_ERROR(dpct::dpct_free(d2_u_old, q_cur())));
      CUDA_CHECK(DPCT_CHECK_ERROR(
          dpct::dpct_free(d2_u_new, q_cur())));
  CUDA_CHECK(
      DPCT_CHECK_ERROR(dpct::dpct_free(d2_rhs, q_cur())));
      CUDA_CHECK(DPCT_CHECK_ERROR(
          dpct::dpct_free(d2_kappa, q_cur())));

  /*
  DPCT1093:316: The "3" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(3)));
  CUDA_CHECK(
      DPCT_CHECK_ERROR(dpct::dpct_free(d3_u_old, q_cur())));
      CUDA_CHECK(DPCT_CHECK_ERROR(
          dpct::dpct_free(d3_u_new, q_cur())));
  CUDA_CHECK(
      DPCT_CHECK_ERROR(dpct::dpct_free(d3_rhs, q_cur())));
      CUDA_CHECK(DPCT_CHECK_ERROR(
          dpct::dpct_free(d3_kappa, q_cur())));
  ...
  printf("\n**** FOUR GPUS CUDA, P2P + 4 HOST THREADS FINISHED ****\n");
  return 0;
}
