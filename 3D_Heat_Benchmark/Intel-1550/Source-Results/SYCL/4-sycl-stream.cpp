...
/* CUDA implementation of the 3D heat conduction problem
   ...
   FOUR-GPU VERSION, ASYNC. Halo exchange staged through the host on
   dedicated streams, overlapped with the interior kernel.
   ...
   This file is kept deliberately parallel to 4-openmp-stream.c and differs
   from 4-cuda.cu only in the asynchrony.
   ...
   STREAM ASSIGNMENT
   -----------------
   THREE streams per device, unlike the two-device file's two:
   ...
   4-openmp-stream.c) so it is resident while the exchanges proceed.

   HOST HALO BUFFERS ARE PAGEABLE, DELIBERATELY
   --------------------------------------------
   See 2-cuda-stream.cu's header: pinned buffers would very likely be faster
   but the OpenMP files stage through plain malloc'd host memory, and pinning
   only the CUDA side would measure a better-tuned algorithm rather than the
   same one.
   ...
   BUILD NOTE:
     nvcc -O3 -arch=sm_90 --fmad=false -Xcompiler -fopenmp 4-cuda-stream.cu -lm -o 4-cuda-stream
*/
...
/*
DPCT1009:186: SYCL uses exceptions to report errors and does not use the error
...
// Identical to 4-openmp-stream.c. Boundaries forced to exact zero.
...
int main(int argc, char **argv) try {
  ...
  /*
  DPCT1093:187: The "0" device may be not the one intended for use. Adjust the
  ...
  */
  ...
  /*
  DPCT1093:188: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  ...
  /*
  DPCT1093:189: The "2" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  ...
  /*
  DPCT1093:190: The "3" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  ...
  for (int g = 0; g < ngpus; ++g) {
    /*
    DPCT1093:191: The "g" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
  ...
  //   streams -- no pack kernel, and no event needed for ordering.
  //
  //   Then every halo stream is synchronised: an H2D reads a host buffer
  //   that a DIFFERENT device's D2H wrote, so all D2H must land first.
  //
  // PHASE 2: H2D into the contiguous ghost planes -- no unpack.
  //
  ...
  while (t < T) {
    ...
    /*
    DPCT1093:192: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
    /*
    ...
    */
    ...
    /*
    DPCT1010:193: SYCL uses exceptions to report errors and does not use the
    ...
    */
    ...
    /*
    DPCT1093:195: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
    /*
    ...
    */
    ...
    /*
    DPCT1010:197: SYCL uses exceptions to report errors and does not use the
    error codes. The call was replaced with 0. You need to rewrite this code.
    */
    CUDA_CHECK(0);
    /*
    ...
    */
    ...
    /*
    DPCT1093:199: The "2" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(2)));
    /*
    ...
    */
    ...
    /*
    DPCT1010:201: SYCL uses exceptions to report errors and does not use the
    error codes. The call was replaced with 0. You need to rewrite this code.
    */
    CUDA_CHECK(0);
    /*
    ...
    */
    ...
    /*
    DPCT1093:203: The "3" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(3)));
    /*
    ...
    */
    ...
    /*
    DPCT1093:206: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
    /*
    DPCT1124:207: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    ...
    */
    ...
    /*
    DPCT1093:208: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
    /*
    DPCT1124:209: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1124:210: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1093:211: The "2" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(2)));
    /*
    DPCT1124:212: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1124:213: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1093:214: The "3" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(3)));
    /*
    DPCT1124:215: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    // -- Every D2H must land before any H2D reads a host buffer --
    /*
    DPCT1093:216: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
    CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_top[0]->wait()));
    /*
    DPCT1093:217: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
    ...
    /*
    DPCT1093:218: The "2" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(2)));
    ...
    /*
    DPCT1093:219: The "3" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(3)));
    ...
    /*
    DPCT1093:220: The "0" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
    /*
    DPCT1124:221: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1093:222: The "1" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
    /*
    DPCT1124:223: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1124:224: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1093:225: The "2" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(2)));
    /*
    DPCT1124:226: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1124:227: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    /*
    DPCT1093:228: The "3" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(3)));
    /*
    DPCT1124:229: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
    the origin API might be synchronous, it depends on the type of operand
    memory, so you may need to call wait() on event return by memcpy API to
    ensure synchronization behavior.
    */
    ...
    // -- Drain every stream on every device before the swap ------
    for (int g = 0; g < ngpus; ++g) {
      /*
      DPCT1093:230: The "g" device may be not the one intended for use. Adjust
      the selected device if needed.
      */
      CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(g)));
      CUDA_CHECK(DPCT_CHECK_ERROR(compute_stream[g]->wait()));
      CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_bottom[g]->wait()));
      CUDA_CHECK(DPCT_CHECK_ERROR(halo_stream_top[g]->wait()));
    }
  ...
  for (int g = 0; g < ngpus; ++g) {
    /*
    DPCT1093:231: The "g" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(g)));
    CUDA_CHECK(
  ...
  /*
  DPCT1093:232: The "0" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
  ...
  /*
  DPCT1093:233: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
  ...
  /*
  DPCT1093:234: The "2" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(2)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
  ...
  /*
  DPCT1093:235: The "3" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(3)));
  CUDA_CHECK(DPCT_CHECK_ERROR(
  ...
  if (should_write_txt())
    write_u_values("u_sycl_stream_4gpu.txt", h_u_global, n);

  // -- Cleanup streams -------------------------------------------
  for (int g = 0; g < ngpus; ++g) {
    /*
    DPCT1093:236: The "g" device may be not the one intended for use. Adjust the
    selected device if needed.
    */
    CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(g)));
    CUDA_CHECK(DPCT_CHECK_ERROR(
  ...
  // -- Cleanup ---------------------------------------------------
  /*
  DPCT1093:237: The "0" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(0)));
  CUDA_CHECK(
  ...
  /*
  DPCT1093:238: The "1" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(1)));
  CUDA_CHECK(
  ...
  /*
  DPCT1093:239: The "2" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(2)));
  CUDA_CHECK(
  ...
  /*
  DPCT1093:240: The "3" device may be not the one intended for use. Adjust the
  selected device if needed.
  */
  CUDA_CHECK(DPCT_CHECK_ERROR(dpct::select_device(3)));
  CUDA_CHECK(
