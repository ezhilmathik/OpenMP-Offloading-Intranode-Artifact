...
int main(int argc, char** argv)
{
  ...
  CUDA_CHECK(cudaMalloc(&d1_u_old, size1));
  CUDA_CHECK(cudaMalloc(&d1_u_new, size1));
  CUDA_CHECK(cudaMalloc(&d1_rhs,   size1));
  CUDA_CHECK(cudaMalloc(&d1_kappa, size1));
  ...
  cudaStream_t halo_stream[ngpus];

  for (int g = 0; g < ngpus; ++g) {
    CUDA_CHECK(cudaSetDevice(g));
    CUDA_CHECK(cudaStreamCreate(&compute_stream[g]));
    CUDA_CHECK(cudaStreamCreate(&halo_stream[g]));
  }
  ...
  while (t < T) {
    ...
    CUDA_CHECK(cudaSetDevice(0));
    CUDA_CHECK(cudaMemcpyAsync(h_halo0_up,
                               d0_u_new + (size_t)(nz0-2) * ny * nx,
                               halo_bytes, cudaMemcpyDeviceToHost,
                               halo_stream[0]));

    CUDA_CHECK(cudaSetDevice(1));
    CUDA_CHECK(cudaMemcpyAsync(h_halo1_dn,
    ...
    CUDA_CHECK(cudaSetDevice(0));
    CUDA_CHECK(cudaStreamSynchronize(halo_stream[0]));
    CUDA_CHECK(cudaSetDevice(1));
    CUDA_CHECK(cudaStreamSynchronize(halo_stream[1]));
   
    
    // -- PHASE 2: H2D into contiguous ghost planes ----------------
    CUDA_CHECK(cudaSetDevice(0));
    CUDA_CHECK(cudaMemcpyAsync(d0_u_new + (size_t)(nz0-1) * ny * nx,
                               h_halo1_dn, halo_bytes,
                               cudaMemcpyHostToDevice, halo_stream[0]));

    CUDA_CHECK(cudaSetDevice(1));
    CUDA_CHECK(cudaMemcpyAsync(d1_u_new + (size_t)0 * ny * nx,
                               h_halo0_up, halo_bytes,
                               cudaMemcpyHostToDevice, halo_stream[1]));
  ...
  CUDA_CHECK(cudaSetDevice(0)); CUDA_CHECK(cudaDeviceSynchronize());
  CUDA_CHECK(cudaSetDevice(1)); CUDA_CHECK(cudaDeviceSynchronize());
  ...
  for (int g = 0; g < ngpus; ++g) {
    ...
    CUDA_CHECK(cudaStreamDestroy(halo_stream[g]));
  }
