...
int main(int argc, char** argv)
{
  ...
  while (t < T) {
    ...
    CUDA_CHECK(cudaSetDevice(0));
    CUDA_CHECK(cudaMemcpyAsync(h_halo0_up, d0_u_new + (size_t)(nz0-2) * ny * nx,
                               halo_bytes, cudaMemcpyDeviceToHost,
                               halo_stream_top[0]));

    CUDA_CHECK(cudaSetDevice(1));
    CUDA_CHECK(cudaMemcpyAsync(h_halo1_dn, d1_u_new + (size_t)1 * ny * nx,
                               halo_bytes, cudaMemcpyDeviceToHost,
                               halo_stream_bottom[1]));
    CUDA_CHECK(cudaMemcpyAsync(h_halo1_up, d1_u_new + (size_t)(nz1-2) * ny * nx,
                               halo_bytes, cudaMemcpyDeviceToHost,
                               halo_stream_top[1]));

    CUDA_CHECK(cudaSetDevice(2));
    CUDA_CHECK(cudaMemcpyAsync(h_halo2_dn, d2_u_new + (size_t)1 * ny * nx,
                               halo_bytes, cudaMemcpyDeviceToHost,
                               halo_stream_bottom[2]));
    CUDA_CHECK(cudaMemcpyAsync(h_halo2_up, d2_u_new + (size_t)(nz2-2) * ny * nx,
                               halo_bytes, cudaMemcpyDeviceToHost,
                               halo_stream_top[2]));

    CUDA_CHECK(cudaSetDevice(3));
    CUDA_CHECK(cudaMemcpyAsync(h_halo3_dn, d3_u_new + (size_t)1 * ny * nx,
                               halo_bytes, cudaMemcpyDeviceToHost,
    ...
    CUDA_CHECK(cudaSetDevice(0));
    CUDA_CHECK(cudaMemcpyAsync(d0_u_new + (size_t)(nz0-1) * ny * nx, h_halo1_dn,
                               halo_bytes, cudaMemcpyHostToDevice,
                               halo_stream_top[0]));

    CUDA_CHECK(cudaSetDevice(1));
    CUDA_CHECK(cudaMemcpyAsync(d1_u_new + (size_t)0 * ny * nx, h_halo0_up,
                               halo_bytes, cudaMemcpyHostToDevice,
                               halo_stream_bottom[1]));
    CUDA_CHECK(cudaMemcpyAsync(d1_u_new + (size_t)(nz1-1) * ny * nx, h_halo2_dn,
                               halo_bytes, cudaMemcpyHostToDevice,
                               halo_stream_top[1]));

    CUDA_CHECK(cudaSetDevice(2));
    CUDA_CHECK(cudaMemcpyAsync(d2_u_new + (size_t)0 * ny * nx, h_halo1_up,
                               halo_bytes, cudaMemcpyHostToDevice,
                               halo_stream_bottom[2]));
    CUDA_CHECK(cudaMemcpyAsync(d2_u_new + (size_t)(nz2-1) * ny * nx, h_halo3_dn,
                               halo_bytes, cudaMemcpyHostToDevice,
                               halo_stream_top[2]));

    CUDA_CHECK(cudaSetDevice(3));
    CUDA_CHECK(cudaMemcpyAsync(d3_u_new + (size_t)0 * ny * nx, h_halo2_up,
                               halo_bytes, cudaMemcpyHostToDevice,
                               halo_stream_bottom[3]));
    ...
    for (int g = 0; g < ngpus; ++g) {
      CUDA_CHECK(cudaSetDevice(g));
      CUDA_CHECK(cudaStreamSynchronize(compute_stream[g]));
      CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[g]));
      CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[g]));
    }
