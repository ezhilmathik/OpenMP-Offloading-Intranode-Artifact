...
int main(int argc, char** argv)
{
  ...
  for (int p = 0; p < 3; p++) {
    ...
    CUDA_CHECK(cudaDeviceCanAccessPeer(&can_ab, a, b));
    CUDA_CHECK(cudaDeviceCanAccessPeer(&can_ba, b, a));
    if (!can_ab || !can_ba) {
      fprintf(stderr, "ERROR: P2P not available between GPU %d and GPU %d; "
                      "this version requires it\n", a, b);
      exit(EXIT_FAILURE);
    }
    CUDA_CHECK(cudaSetDevice(a)); CUDA_CHECK(cudaDeviceEnablePeerAccess(b, 0));
    CUDA_CHECK(cudaSetDevice(b)); CUDA_CHECK(cudaDeviceEnablePeerAccess(a, 0));
  ...
  const int ngpus = 4;
  cudaStream_t compute_stream[ngpus];
  cudaStream_t halo_stream_bottom[ngpus];
  cudaStream_t halo_stream_top[ngpus];

  for (int g = 0; g < ngpus; ++g) {
    CUDA_CHECK(cudaSetDevice(g));
    CUDA_CHECK(cudaStreamCreate(&compute_stream[g]));
    CUDA_CHECK(cudaStreamCreate(&halo_stream_bottom[g]));
    CUDA_CHECK(cudaStreamCreate(&halo_stream_top[g]));
  }
...
#pragma omp parallel num_threads(ngpus)
  {
    ...
    while (t < T) {
      ...
      if (tid == 0) {
        ...
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[0]));
      }
      else if (tid == 1) {
        ...
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[1]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[1]));
      }
      else if (tid == 2) {
        ...
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[2]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[2]));
      }
      else {
        ...
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[3]));
      }
...
#pragma omp barrier
      ...
      if (tid == 0) {
        ...
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[0]));
        CUDA_CHECK(cudaStreamSynchronize(compute_stream[0]));
      }
      else if (tid == 1) {
        CUDA_CHECK(cudaMemcpyPeerAsync(
        ...
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[1]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[1]));
        CUDA_CHECK(cudaStreamSynchronize(compute_stream[1]));
      }
      else if (tid == 2) {
        CUDA_CHECK(cudaMemcpyPeerAsync(
          d2_u_new + (size_t)0       * ny * nx, 2,   // dst: GPU2 bottom ghost
          d1_u_new + (size_t)(nz1-2) * ny * nx, 1,   // src: GPU1 plane k=nz1-2
        ...
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[2]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_top[2]));
        CUDA_CHECK(cudaStreamSynchronize(compute_stream[2]));
      }
      else {
        CUDA_CHECK(cudaMemcpyPeerAsync(
          d3_u_new + (size_t)0       * ny * nx, 3,   // dst: GPU3 bottom ghost
          d2_u_new + (size_t)(nz2-2) * ny * nx, 2,   // src: GPU2 plane k=nz2-2
          halo_bytes, halo_stream_bottom[3]));
        CUDA_CHECK(cudaStreamSynchronize(halo_stream_bottom[3]));
        CUDA_CHECK(cudaStreamSynchronize(compute_stream[3]));
      }
...
#pragma omp single
  ...
  for (int g = 0; g < ngpus; ++g) {
    CUDA_CHECK(cudaSetDevice(g));
    CUDA_CHECK(cudaStreamDestroy(compute_stream[g]));
    CUDA_CHECK(cudaStreamDestroy(halo_stream_bottom[g]));
    CUDA_CHECK(cudaStreamDestroy(halo_stream_top[g]));
  }
  ...
  for (int p = 0; p < 3; p++) {
    int a = p2p_pairs[p][0], b = p2p_pairs[p][1];
    CUDA_CHECK(cudaSetDevice(a)); cudaDeviceDisablePeerAccess(b);
    CUDA_CHECK(cudaSetDevice(b)); cudaDeviceDisablePeerAccess(a);
  }
