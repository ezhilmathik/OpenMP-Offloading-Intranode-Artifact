/* ===========================================================================
 * 2-omp-halo.c -- Conjugate Gradient, unpreconditioned, OpenMP target offload,
 *                 2 GPUs (single node), REAL PACKED HALO.
 *
 * ========================================================================= */
...
/* ===========================================================================
 * CG -- unpreconditioned, standard formulation. Identical to 2-omp.c.
 *
 *   r = b - A x ;  p = r ;  rr = r.r
 *   repeat
...
#pragma omp parallel for schedule(static)
  ...
  /* send-side offsets and concatenated gather lists.
   * SOFF[e][d] must select the SAME block, in the SAME order, that ROFF[d][e]
   * selects on the other end -- both are L[d][e], which is why this works. */
  for (e = 0; e < NGPU; e++) {
    int off = 0;
