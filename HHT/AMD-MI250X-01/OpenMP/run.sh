...
run_bench() {
    ...
    if [[ "${NO_TIMEOUT:-0}" == "1" ]]; then
        echo "    timeout wrapper: disabled"
        exec_cmd=("${run_cmd[@]}")
    else
        echo "    timeout wrapper: ${TIMEOUT_SEC}s"
        exec_cmd=(timeout "$TIMEOUT_SEC" "${run_cmd[@]}")
    fi

    if "${exec_cmd[@]}" > "$stdout_file" 2>&1; then
        status="ok"
        exit_code=0
    else
        exit_code=$?
        if [[ "${NO_TIMEOUT:-0}" != "1" && $exit_code -eq 124 ]]; then
            status="timeout"
        else
...
if [[ -n "$RUN_ONLY" ]]; then
    echo "Run-only selection: $RUN_ONLY"

    case "$RUN_ONLY" in
        1-omp)
            run_bench "$RUN_ONLY" 1
            ;;

        2-omp|2-omp-stream|2-omp-stream-omp|2-omp-stream-omp-p2p)
            run_bench "$RUN_ONLY" 2
            ;;

        4-omp|4-omp-stream|4-omp-stream-omp|4-omp-stream-omp-p2p)
            run_bench "$RUN_ONLY" 4
            ;;

        *)
            echo "ERROR: unsupported RUN_ONLY value: $RUN_ONLY" >&2
            exit 1
            ;;
    esac
else
